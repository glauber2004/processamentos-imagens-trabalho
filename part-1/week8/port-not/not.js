...
window.onload = function() {
    ...
    let binCanvas = document.getElementById('binCanvas');
    let originalContext = originalCanvas.getContext('2d');
    let binContext = binCanvas.getContext('2d');
    ...
    function treatImage(changeEvent) {
        ...
        reader.onload = function(event) {
            ...
            img.onload = function() {
                ...
                originalCanvas.width = binCanvas.width = img.width;
                originalCanvas.height = binCanvas.height = img.height;
    ...
    window.invertBin = function() {
        ...
        for (let y = 0; y < originalCanvas.height; y++) {
            for (let x = 0; x < originalCanvas.width; x++) {
                ...
                let test =  (r > 127) && (g > 127) && (b > 127) 
                let invertvalue = test ? 0 : 255;
                 
                
    

                // Define os valores R, G, B para o valor de cinza
                pixels[index] = pixels[index + 1] = pixels[index + 2] = invertvalue;
            }
        }

        // Coloca os dados de volta no canvas de escala de cinza
        binContext.putImageData(imageData, 0, 0);
    };
}
...
function downloadImage() {
    ...
    downloadLink.href = binCanvas.toDataURL('image/jpg');
    downloadLink.download = 'image_not.jpg';
    downloadLink.click();
}
